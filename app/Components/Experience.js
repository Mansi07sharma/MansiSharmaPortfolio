      ...
      <div className="container mx-auto lg:px-24 px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 justify-center">
          ...
          <div className="lg:col-span-2">
            <ExpCard exp={experiences[0]} idx={idx} setIdx={setIdx} index={0} handleOpen={handleOpen} />
          </div>
...
const ExpCard = ({ exp, idx, setIdx, index, handleOpen }) => (
  <div className="bg-[#242323] md:p-8 p-4 rounded-xl border-l-4 border-r-4 border-gray-400 h-full flex flex-col">
    ...
    <ul className={`space-y-3 ${idx === index ? 'block' : 'hidden'} md:block transition-all duration-300`}>
      {exp.achievements.map((achievement, idx2) => (
        <li key={idx2} className="flex items-start space-x-3">
